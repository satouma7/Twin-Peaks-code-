...
nt=1;K1=0.3;Knt=K1^nt;%nt: Hill's co-efficient@kt:activation co-efficient  for trans-activation
nc=5;K2=0.1;Knc=K2^nc;%nc: Hill's co-efficient@kc:activation co-efficient for cis-inhibition
...
ESH= zeros(Xmax,Xmax);
DNc= zeros(Xmax,Xmax);
SigmaD= zeros(Xmax,Xmax);SigmaDNt=zeros(Xmax,Xmax);Hill=zeros(Xmax,Xmax);
...
for T=1: Tmax-1 
    ...
    S(:,:,T+1)= S(:,:,T) + dt*( dn* Hill.*F(:,:,T)  -ks*S(:,:,T) );  
    
    DNc=D(:,:,T).^nc;
    F(:,:,T+1)= F(:,:,T) + dt*( n*(1-F(:,:,T)) - dc*DNc./(Knc+DNc).*F(:,:,T) -kf*F(:,:,T) -kt*Hill.*F(:,:,T) );  
...
for T=1:20:800
    ...
    subplot(3,2,6);
    plot(Xsteps,A(13,:,T),'b',Xsteps, 5*E(13,:,T),'k',Xsteps,5*D(13,:,T),'g',Xsteps,10*S(13,:,T),'r',Xsteps,F(13,:,T),'m');ylim([-0.1 1.2]);
    pause(0.001);
end